...
const MapComponent = ({ faskes, kodeCabang }) => {
  ...
  }, [bboxCabang, map, potentialLayerUrl]);
  ...
  const handleSubmit = () => {
    ...
    const sanitizedKodeCabang = kodeCabang ?? "null";
    ...
    if (
      ...
      dispatch(fetchCenterCabang(sanitizedKodeCabang));
    ...
    <Box className="contentRoot">
      ...
      {showKoordinat && (
        <div className="box-koordinat">
          ...
          >
            <CloseIcon />{" "}
            {/* Add the CloseIcon component or use another icon */}
          </IconButton>
        </div>
      )}
      {isFKTPAll ? (
